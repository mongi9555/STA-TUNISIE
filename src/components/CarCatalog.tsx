...
import { Check, ShieldCheck, Zap, Sparkles, Plus, Car, Fuel, Settings2, Info, X, Gauge, Shield, Award, Maximize2, FileText, Download, Eye, Calendar, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-red-950/40 border border-slate-800 rounded-2xl p-6 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-300 max-w-2xl mt-1">
            Sélectionnez un modèle et sa couleur d'origine pour vérifier le stock en temps réel et établir immédiatement un Bon de Réservation pour votre client.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cars.map((car) => {
          const selectedColorId = selectedColorMap[car.id] || car.colors[0]?.id;
          const selectedColor = car.colors.find((c) => c.id === selectedColorId) || car.colors[0];
          const totalStock = car.colors.reduce((acc, c) => acc + c.stock, 0);

          // Commercial Session Quota calculation (Quota = 5 max / model)
          const quotaPerModel = currentUser?.quotaPerModel || 5;
          const modelReservationsCount = (reservations || []).filter(
            (r) =>
              currentUser &&
          ...
          const isQuotaReached = modelReservationsCount >= quotaPerModel;
                    ...
                    {currentUser && (
                      <span className={`px-2.5 py-1 text-xs font-bold rounded-lg shadow backdrop-blur flex items-center gap-1 border ${
                        isQuotaReached
                          ...
                          : modelReservationsCount > 0
                          ? 'bg-amber-950/90 text-amber-300 border-amber-500/50'
                          : 'bg-emerald-950/90 text-emerald-300 border-emerald-500/40'
                      }`}>
                        <ShieldCheck className="w-3.5 h-3.5" />
                        Quota: {modelReservationsCount}/{quotaPerModel}
                      </span>
                    )}
                  </div>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="p-2 bg-emerald-950/40 border border-emerald-500/30 rounded-xl text-[11px] text-emerald-200 flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400">Véhicule {car.priceTND.toLocaleString()} DT + Frais immat/carte grise/timbre {getRegistrationFeeForCar(car).toLocaleString()} DT</span>
                    </div>
              ...
              <div className="p-4 bg-slate-950 border-t border-slate-800 space-y-2">
                ...
                <button
                  ...
                  className={`w-full py-2.5 px-4 rounded-xl text-xs font-extrabold flex items-center justify-center gap-2 transition-all shadow-md ${
                    isQuotaReached
                      ? 'bg-rose-950/80 hover:bg-rose-900 border border-rose-500/50 text-rose-200 cursor-pointer'
                ...
                >
                  {isQuotaReached ? (
                    <>
                      ...
                      <span>Quota Atteint ({modelReservationsCount}/{quotaPerModel}) — Voir Bon</span>
                    </>
                  ) : (
                    <>
